...
Initial release of the LoanContractTerms schema (`contractAttributes`) for the "Personal Loan" use case, converted to Beckn v2.0.0 LTS. Carries `borrowerTerms`/
`lenderTerms` (each a `staticTermsUrl` + `offlineContract` flag), attached to the
...
`borrowerTerms`/`lenderTerms` data carried at confirm time) — see `../../README.md` and
